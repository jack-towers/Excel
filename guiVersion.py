...
def process_service_status():
    ...
    if not csv_path:  # If no file is selected, exit the program
        print("No file selected. Exiting...")
        return

    # Prompt user to select the output file location for the Excel file
    output_path = filedialog.asksaveasfilename(title="Save Excel File As", defaultextension=".xlsx", filetypes=(("Excel Files", "*.xlsx"), ("All Files", "*.*")))

    if not output_path:  # If no file is selected, exit the program
    ...
    for row in ws.iter_rows(min_row=5, max_row=ws.max_row, min_col=weeks_overdue_col_idx, max_col=weeks_overdue_col_idx):
        for cell in row:
    ...
    # At the end of your process, just before saving the Excel file
    for col in ["Date of Last Service", "Next Service Due"]:
        df[col] = df[col].dt.strftime("%Y-%m-%d")  # or "%d-%b-%Y" for "01-Jan-2025" format
